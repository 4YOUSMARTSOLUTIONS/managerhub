-- Colunas para auto-reserva de séries recorrentes
...
-- Gera/atualiza as reservas de sala (meetings) de UMA série, 12 meses à frente.
-- SQL puro para poder rodar via pg_cron sem depender do app.
...
-- Top-up de todas as séries ativas com auto-reserva (alvo do pg_cron)
...
declare r record;
begin
  for r in select id from public.meeting_series where is_active and auto_book loop
