...
-- =============================================================
-- MANAGERHUB · Migration 18 · RPCs de funcionário + login por CPF
-- =============================================================

-- ---------- Login por CPF: resolve o e-mail de autenticação ----------
create or replace function public.email_by_cpf(p_cpf text)
returns text language sql security definer stable set search_path = public as $$
  select u.email
  from auth.users u
  join public.profiles p on p.id = u.id
  where p.cpf = regexp_replace(coalesce(p_cpf, ''), '[^0-9]', '', 'g')
    and length(regexp_replace(coalesce(p_cpf, ''), '[^0-9]', '', 'g')) = 11
  limit 1;
$$;
revoke all on function public.email_by_cpf(text) from public;
grant execute on function public.email_by_cpf(text) to anon, authenticated;

-- ---------- Criar funcionário (auth + perfil + vínculo + unidades) ----------
...
begin
  ...
  if p_password is null or length(p_password) < 6 then raise exception 'Senha mínima de 6 caracteres'; end if;
  ...
  insert into public.memberships (
    ...
    department_id, subdepartment_id, position_id, position_level_id, manager_id
  ) values (
    v_tenant, v_uid,
...
revoke all on function public.admin_create_employee(jsonb, text) from public, anon;
revoke all on function public.admin_update_employee(uuid, jsonb) from public, anon;
...
-- demo: CPF do admin para permitir login por CPF (111.444.777-35)
update public.profiles set cpf = '11144477735' where email = 'admin@managerhub.app' and cpf is null;
