-- #1: índice do hot path (exists()/delete por slot no sync)
create index if not exists idx_meetings_series_slot on public.meetings (series_id, series_slot);
-- suporte ao is_holiday(tenant, day) chamado no loop de deslocamento
create index if not exists idx_holidays_tenant_day on public.holidays (tenant_id, day);

-- #4: serializar sync por série (cron + save simultâneos) e estreitar o catch
...
begin
  ...
  -- serializa geração da MESMA série (evita corrida cron x save)
  perform pg_advisory_xact_lock(hashtext('series_bookings:' || p_series::text));
  ...
  foreach v_slot in array v_targets loop
    if not exists (select 1 from public.meetings where series_id = p_series and series_slot = v_slot) then
      ...
      exception when unique_violation or exclusion_violation then
        null; -- slot já ocupado / conflito de sala: pula só esta ocorrência
      end;
    end if;
  end loop;
end; $$;
