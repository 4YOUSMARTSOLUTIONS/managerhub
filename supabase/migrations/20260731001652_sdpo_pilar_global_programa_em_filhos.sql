...
-- backfill do programa a partir do pilar atual (ANTES de mesclar os pilares)
...
-- dedupe de pilares por (tenant, nome normalizado): sobrevive o mais antigo.
...
-- pilar deixa de pertencer a um programa (a informação vive nos filhos)
alter table public.sdpo_pilares drop column if exists programa_id;

-- impedir novos pilares duplicados por nome (por tenant)
