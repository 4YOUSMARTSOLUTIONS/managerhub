...
begin
  ...
  -- monta a grade; se cair em dia não útil (domingo ou feriado nacional/custom),
  -- desloca para o PRÓXIMO DIA ÚTIL mantendo o horário. Deduplica colisões.
  ...
  foreach v_slot in array v_targets loop
    if not exists (select 1 from public.meetings where series_id = p_series and starts_at = v_slot) then
      begin
        ...
        values (s.tenant_id, s.name, s.objetivo,
                ...
                s.owner_user_id, s.owner_user_id, v_slot, v_slot + v_dur, p_series, 'scheduled')
