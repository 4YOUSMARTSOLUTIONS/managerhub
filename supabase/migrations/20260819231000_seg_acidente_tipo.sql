-- O acidente ganha a segunda classificação, e o painel ganha o filtro dela.
--
-- `tipo` entra com default 'tipico': as linhas que já existem são todas de
-- acidente na operação, então o default classifica o passado corretamente em
-- vez de deixar um campo nulo para alguém preencher depois.
--
-- No painel os dois contam JUNTOS por padrão, e o filtro é opcional. Ele mora
-- na RPC, e não no cliente, porque os números do painel são agregados no banco
-- (foi assim que o módulo nasceu, para a tela não fazer dez idas ao banco);
-- filtrar depois, no navegador, só funcionaria para a lista, nunca para a
-- pirâmide.
--
-- O filtro toca UM ponto: o CTE `acidentes`. Pirâmide, dias perdidos,
-- por_classe, mensal, por_local, por_area, por_causa e o bloco restrito leem
-- todos dele, então passam a respeitar o recorte de graça. Relatos ficam
-- inteiros: eles não têm típico nem trajeto, e a tela avisa isso quando o
-- filtro está ativo.
--
...
-- chamada. Mesma lição de `seg_triar_relato`.

alter table public.seg_acidentes
  add column tipo public.seg_acidente_tipo not null default 'tipico';

create index seg_acidentes_tipo_idx
  on public.seg_acidentes (tenant_id, tipo, occurred_on desc);

drop function if exists public.seg_dashboard(integer, uuid[]);

create function public.seg_dashboard(
  p_ano integer,
  p_unit_ids uuid[] default null,
  p_tipo public.seg_acidente_tipo default null
)
returns jsonb
language plpgsql
stable
  ...
  acidentes as (
    select a.*
     ...
     where a.tenant_id = v_tenant
       ...
       -- nulo = os dois juntos, que e o padrao da tela
    ...
    acidentes as (
      select a.* from public.seg_acidentes a
       where a.tenant_id = v_tenant
         and a.occurred_on between v_de and v_ate
         and (p_unit_ids is null or a.snap_unit_id = any(p_unit_ids) or a.snap_unit_id is null)
         -- o bloco restrito segue o mesmo recorte do painel de cima
         and (p_tipo is null or a.tipo = p_tipo)
    )
    select jsonb_build_object(
      'taxa_tratamento', (
...
grant  execute on function public.seg_dashboard(integer, uuid[], public.seg_acidente_tipo) to authenticated;
