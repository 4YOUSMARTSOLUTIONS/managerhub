-- Importação em lote passa a aceitar o PERFIL de acesso.
--
-- Vem logo depois da coluna de gestor pelo mesmo motivo: marcar ~100 Gestores um
-- a um na tela é inviável, e a planilha que já traz o organograma é o lugar
-- natural para dizer quem lidera. Sem isso, a trava por papel entregue na
-- migração anterior ficaria sem como ser ligada em escala.
--
-- TETO DE PRIVILÉGIO, de propósito: a planilha só alcança `member`, `team_lead`
-- e `manager`. "Administrador" e "Proprietário" são recusados com erro. Uma
-- linha de Excel não pode ser o caminho para alguém virar administrador da
-- empresa: essas duas promoções continuam exigindo a tela, com uma pessoa
-- decidindo de propósito. Vale mesmo com a RPC já sendo owner/admin.
--
-- Célula vazia não muda o perfil de ninguém, mesma regra da coluna de gestor.
-- Numa planilha de 987 linhas, o vazio é o estado mais provável, e ele tem de
-- significar "não mexe".
--
-- Perfil e gestor são contados separados no resumo. Uma linha pode mudar os
-- dois, e juntar tudo em "recontratados" esconderia o que de fato aconteceu.
...
declare
  ...
  v_managers int := 0; v_roles int := 0;
...
begin
  ...
  for r in select value from jsonb_array_elements(p_rows) as t(value) loop
    begin
      ...
      if v_existing_uid is not null then
        select id, employee_code, admission_date, dismissed_at, department_id, subdepartment_id, position_id, position_level_id, manager_id, role
          into v_existing_mid, v_existing_code, v_existing_adm, v_ex_dis, v_ex_dept, v_ex_sub, v_ex_pos, v_ex_lvl, v_cur_mgr, v_cur_role
        ...
        if v_existing_mid is null then
          ...
          if (v_mgr_given and v_cur_mgr is distinct from v_mgr)
             or (v_role_given and v_cur_role is distinct from v_role) then
            ...
            v_skipped_list := v_skipped_list || jsonb_build_object(
              ...
              'motivo', case when v_mgr_given or v_role_given then 'Já cadastrado, nada a mudar'
        ...
        values (v_tenant, v_uid, coalesce(v_role, 'member'), v_code, v_adm, v_dept, v_sub, v_pos, v_lvl, v_mgr, v_dismissed is null, v_dismissed)
        returning id into v_mid;
        v_created := v_created + 1;
      end if;
  ...
  return jsonb_build_object(
    ...
    'managers', v_managers, 'roles', v_roles,
    'skippedList', v_skipped_list, 'updatedList', v_updated_list,
    'managersList', v_managers_list, 'rolesList', v_roles_list, 'errors', v_errors);
end $function$;
