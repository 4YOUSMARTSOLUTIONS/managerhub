-- Importação de ações para MIGRAÇÃO (owner): grava data de criação, criador, reunião,
-- status e data de conclusão históricos, em vez de carimbar "agora".
...
begin
  ...
  insert into public.action_demanda_assignees (demanda_id, user_id, completed_at)
  select v_dem, x::uuid, case when v_status = 'done' then coalesce(v_completed, v_created) else null end
