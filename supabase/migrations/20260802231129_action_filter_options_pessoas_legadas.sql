-- Solicitante e Responsável passam a marcar quem não está mais ativo na empresa:
-- nomes que vieram como texto da migração ou pessoas com vínculo inativo. A tela
-- separa esses num grupo "Legados", em cinza, depois de quem está ativo hoje.
--
-- Substituída logo em seguida por 20260802231246 (blindagem do NOT IN).
...
-- quem está no quadro hoje; tudo que não bate por nome é tratado como legado
ativos as (
  select pr.full_name as nm
  from public.memberships m
  join public.profiles pr on pr.id = m.user_id
  cross join ctx
  where m.tenant_id = ctx.tenant and m.is_active
),
vis as (
