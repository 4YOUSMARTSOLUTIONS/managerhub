-- Só o dono do SaaS (super admin) pode conceder/alterar/remover o papel de Proprietário (owner),
-- e a empresa nunca pode ficar sem nenhum owner.
...
begin
  ...
  if exists (select 1 from public.memberships where user_id = p_user and tenant_id = v_tenant and role = 'owner') then
    if not public.is_super_admin() then
      raise exception 'Apenas o proprietário do sistema pode excluir um proprietário';
    end if;
    if (select count(*) from public.memberships where tenant_id = v_tenant and role = 'owner') <= 1 then
      raise exception 'Cadastre outro proprietário antes de excluir este';
    end if;
  end if;
