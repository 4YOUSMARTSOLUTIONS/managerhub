-- Programa (SPO/DPO) na ação. Até aqui o programa só existia em blocos/itens, então
-- ação sem bloco/item (caso da migração, que traz só o Pilar) ficava sem programa.
-- Ação nativa: derivado do item/bloco escolhido. Migração: resolvido do arquivo (ou texto legacy).

alter table public.actions
  add column if not exists programa_id uuid references public.sdpo_programas(id) on delete set null,
  add column if not exists legacy_programa text;

-- ação nativa: grava o programa do item (ou do bloco) escolhido na cascata
...
-- migração: grava o programa vindo do arquivo (vinculado quando existe, texto quando não)
...
begin
  ...
  insert into public.actions (
    ...
    legacy_created_by, legacy_meeting, legacy_unit, legacy_kpi, legacy_tool, legacy_programa
  ) values (
    v_tenant, v_sdpo,
    ...
    (select id from public.sdpo_programas where id = nullif(p_data->>'programa_id','')::uuid and tenant_id = v_tenant),
    (select id from public.meeting_series where id = nullif(p_data->>'meeting_series_id','')::uuid and tenant_id = v_tenant),
    (select id from public.action_kpis where id = nullif(p_data->>'kpi_id','')::uuid and tenant_id = v_tenant),
    (select id from public.action_tools where id = nullif(p_data->>'tool_id','')::uuid and tenant_id = v_tenant),
    (select id from public.units where id = nullif(p_data->>'unit_id','')::uuid and tenant_id = v_tenant),
    ...
    nullif(p_data->>'legacy_tool',''), nullif(p_data->>'legacy_programa','')
  ) returning id into v_action;
