alter table public.actions add column if not exists unit_id uuid references public.units(id) on delete set null;
create index if not exists idx_actions_unit on public.actions (unit_id);
