...
-- Status ativo/inativo do colaborador no sistema (demitido = inativo)
alter table public.memberships add column if not exists is_active boolean not null default true;
alter table public.memberships add column if not exists dismissed_at date;

-- Importação passa a aceitar dismissed_at -> cria inativo
...
begin
  ...
  for r in select value from jsonb_array_elements(p_rows) as t(value) loop
    begin
      ...
      v_unit := null; v_name := nullif(trim(r->>'unit'), '');
      ...
      v_dismissed := nullif(r->>'dismissed_at', '')::date;
      ...
      values (v_tenant, v_uid, 'member', nullif(trim(r->>'employee_code'), ''), nullif(r->>'admission_date', '')::date, v_dept, v_sub, v_pos, v_lvl, v_dismissed is null, v_dismissed)
      returning id into v_mid;

      if v_unit is not null then insert into public.membership_units (membership_id, unit_id) values (v_mid, v_unit) on conflict do nothing; end if;
