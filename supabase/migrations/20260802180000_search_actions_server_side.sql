-- Filtros de Ações no servidor: a tela filtrava só a página carregada (50 itens).
-- Estas funções filtram a base inteira e devolvem os ids da página + o total real.
-- SECURITY INVOKER de propósito: a RLS de actions/action_demandas continua valendo.

create extension if not exists unaccent with schema extensions;

-- normalização igual à do cliente: minúsculas e sem acentos
create or replace function public.mh_norm(p text)
returns text
language sql
stable
parallel safe
set search_path to 'public'
as $function$ select lower(extensions.unaccent(coalesce(p, ''))) $function$;

-- status efetivo da demanda, mesma regra do effStatus() em src/lib/constants.ts
create or replace function public.demanda_eff_status(p_demanda uuid)
returns text
language sql
stable
set search_path to 'public'
as $function$
  select case
    ...
    when exists (
      select 1 from public.action_demanda_assignees s
