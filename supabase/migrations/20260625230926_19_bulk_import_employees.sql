...
-- =============================================================
-- MANAGERHUB · Migration 19 · Importação em lote de colaboradores
-- =============================================================
...
declare
  ...
  v_unit uuid; v_dept uuid; v_sub uuid; v_pos uuid; v_lvl uuid;
...
begin
  ...
  for r in select value from jsonb_array_elements(p_rows) as t(value) loop
    begin
      ...
      -- Unidade (get-or-create)
      v_unit := null;
      v_name := nullif(trim(r->>'unit'), '');
      ...
      -- Setor
      v_dept := null;
      v_name := nullif(trim(r->>'department'), '');
      if v_name is not null then
      ...
      -- Subsetor (ignora "Não informado")
      v_sub := null;
      v_name := nullif(trim(r->>'subdepartment'), '');
      ...
      -- Perfil de função (ignora vazio ou textos enormes)
      v_lvl := null;
      v_name := nullif(trim(r->>'level'), '');
      ...
      insert into auth.users (
        ...
        v_auth_email, v_hash, now(), now(), now(),
        '{"provider":"email","providers":["email"]}', jsonb_build_object('full_name', trim(r->>'full_name')),
        '', '', '', ''
      );
      ...
      values (v_uid, v_email, trim(r->>'full_name'), v_cpf, nullif(trim(r->>'phone'), ''), nullif(r->>'birth_date', '')::date, v_gender)
      ...
      insert into public.memberships (tenant_id, user_id, role, employee_code, admission_date, department_id, subdepartment_id, position_id, position_level_id)
      values (v_tenant, v_uid, 'member', nullif(trim(r->>'employee_code'), ''), nullif(r->>'admission_date', '')::date, v_dept, v_sub, v_pos, v_lvl)
...
revoke all on function public.admin_import_employees(jsonb, text) from public, anon;
