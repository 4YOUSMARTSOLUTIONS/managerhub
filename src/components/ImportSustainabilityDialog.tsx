...
export function ImportSustainabilityDialog({ open: openProp, onClose, hideTrigger }: { open?: boolean; onClose?: () => void; hideTrigger?: boolean } = {}) {
  const [internalOpen, setInternalOpen] = useState(false);
  const controlled = openProp !== undefined;
  const open = controlled ? openProp : internalOpen;
  const [rows, setRows] = useState<SustImportRow[]>([]);
  const [fileName, setFileName] = useState("");
  const [parseError, setParseError] = useState("");
  const [importing, setImporting] = useState(false);
  const [summary, setSummary] = useState<SustImportResult | null>(null);
  ...
  async function downloadTemplate() {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      [2, "OTIF", "%", "João Silva", 0.92, "maior é melhor", "razão"],
    ]);
    ws["!cols"] = [{ wch: 10 }, { wch: 34 }, { wch: 12 }, { wch: 26 }, { wch: 10 }, { wch: 16 }, { wch: 18 }];
    const wsI = XLSX.utils.aoa_to_sheet([
      ["Coluna", "Como preencher"],
      ["Ordem KPI", "Número de ordenação do KPI"],
      ["KPI", "Nome do indicador"],
      ["Un. Medida", "%, N°, Un., pontos…"],
      ["DONO", "Nome completo do responsável, como cadastrado"],
      ["META", "Valor da meta (metas em % podem ser fração, ex.: 0,92 = 92%)"],
      ["Direção", "\"Maior é melhor\" ou \"Menor é melhor\""],
      ["Cálculo acumulado", "Soma, Razão (Σnº ÷ Σtotal), Média ou Manual"],
    ]);
    wsI["!cols"] = [{ wch: 20 }, { wch: 70 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "KPIs");
    XLSX.utils.book_append_sheet(wb, wsI, "Instruções");
    XLSX.writeFile(wb, "modelo_kpis_sustentabilidade.xlsx");
  }
    ...
    try {
      ...
      for (let i = 1; i < aoa.length; i++) {
        ...
        parsed.push({
          ...
          target: cell(r, targetIdx) as string | number,
          direction: str(r, dirIdx),
          consolidation: str(r, consIdx),
        });
      }
  ...
  async function doImport() {
    setImporting(true);
    const res = await importSust(rows);
  ...
  return (
    <>
      {!hideTrigger && <button type="button" className="btn btn-ghost" onClick={() => setInternalOpen(true)}><IconImport /> Importar KPIs</button>}
      {open && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(3, 6, 14, 0.6)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "5vh 1rem", zIndex: 70, overflowY: "auto" }}>
          <div className="card" style={{ width: "100%", maxWidth: 560, boxShadow: "var(--mh-shadow-e3)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", borderBottom: "1px solid var(--border)" }}>
              <h2 style={{ fontSize: "1.05rem", fontWeight: 700, margin: 0 }}>Importar KPIs de Sustentabilidade (.xlsx)</h2>
            ...
            <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              {fileName && !summary && (
                <div className="card card-pad" style={{ fontSize: "0.88rem" }}>
                  ...
                  <div className="muted" style={{ marginTop: 4 }}>{rows.length} KPI(s)</div>
                </div>
              )}
              {summary && (
                  ...
                  {summary.error ? <span className="badge badge-red">{summary.error}</span> : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                        <span className="badge badge-green">{summary.kpis} KPI(s)</span>
                        {summary.skipped > 0 && <span className="badge badge-amber">{summary.skipped} já existia(m)</span>}
                      </div>
                      {summary.ownersNotFound.length > 0 && <div className="muted" style={{ fontSize: "0.82rem" }}>Responsáveis não encontrados (ajuste depois): {summary.ownersNotFound.join(", ")}</div>}
                    </div>
                  )}
                </div>
              )}
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.6rem", padding: "1rem 1.25rem", borderTop: "1px solid var(--border)" }}>
              ...
              <button type="button" className="btn btn-primary" disabled={!rows.length || importing || (!!summary && !summary.error)} onClick={doImport}>{importing ? "Importando…" : "Importar"}</button>
            </div>
          </div>
        </div>
      )}
