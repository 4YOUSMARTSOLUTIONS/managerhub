...
export function NpsRating({ ticketId, current, currentComment, onSubmitted }: { ticketId: string; current: number | null; currentComment: string | null; onSubmitted?: () => void }) {
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 9, padding: "0.8rem 0.9rem", background: "var(--surface-2)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.6rem" }}>
        ...
        <span style={{ fontSize: "0.76rem", color: "var(--mh-success)", opacity: saved ? 1 : 0, transition: "opacity 0.2s" }}>✓ Avaliação enviada</span>
      </div>
      ...
      {error && <p style={{ color: "var(--mh-danger)", fontSize: "0.8rem", margin: "0.4rem 0 0" }}>{error}</p>}
